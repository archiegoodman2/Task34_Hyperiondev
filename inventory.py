...
Quantity = {self.quantity}''')
    ...
    def original_details(self):                   #this method can be used to repopulate the database 
        line = str(self.country)+','+str(self.code)+','+str(self.product)+','+str(self.cost)+','+str(self.quantity)+'\n'
        return line
...
def re_stock():
    ...
    if choice == 'y' :
        ...
        new_contents = "Country,Code,Product,Cost,Quantity\n"           #this will create our new contents string and add our first line, which is always the same.
...
def value_per_item():
    ...
    choice = input('''
    Would you like to get the price of an individual product, by name? Y/N
    ''').strip(" ").lower()

    if choice == 'y' :
        choice2 = input("Please enter the exact name of the shoe you would like to view the price for: ").strip(" ")
        try: 
            print(f" The price of {choice2} is: {values_dict[choice2]}")
        except KeyError:
...
while True:
    ...
    #                                                                                             #
    #  c - capture shoes (add a new product description to the database)                          #
    #  vi - view all products in the database                                                     #
    #  r - re-stock the lowest-stocked shoe on the database                                       #
    #  s - search for details of a given shoe on record                                           #
    #  va - browse the value of the products in stock                                             #
    #  h - view the shoe with the highest quantity on record currently                            #
    #  e - exit                                                                                   #
    #                                                                                             #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
